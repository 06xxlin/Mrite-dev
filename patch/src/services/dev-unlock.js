...
//
// 调用时机：core/bootstrap.js 中，在 authService.register() 之后 install()。
//   （register() 会注册/重注册这些授权 IPC，必须先让它跑完再覆写。）
//
// 关键点：权限判定走的是 authService 的「运行时属性查找」
//   · services/task/index.js  → authService.verifySessionForOperation()
//   · services/task/runner.js → authService.isLicenseValid()
//   · services/task/index.js  → authService.getSessionToken()（无 token 时走自有 API Key 分支）
...
function install() {
  ...
  // ── 4. 主动广播，压制 register() 内部 2s/3s 的联网校验结果 ──
  [2500, 5000, 8000, 15000].forEach((t) => setTimeout(broadcastAuthorized, t));

  note('开发版解锁已生效：登录 / 激活码 / 会员时长校验全部放行');
  note('任务使用「设置 → 模型配置」中的自有 API Key 直连（不走云端代理）');
}

module.exports = { install, broadcastAuthorized };
